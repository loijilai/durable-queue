...
# Job 佇列（SQS；本機為 ElasticMQ）。API 送出訊息、Worker 調整 visibility 都用這個 URL。
JOB_QUEUE_URL = os.environ["JOB_QUEUE_URL"]


# Structured logging
# 每一行 JSON 帶 job id（由 jobs.worker.handler 在執行期間寫入 contextvar，
# 不需要每個呼叫點自己傳遞）。容器日誌交由容器平台原生的日誌驅動
