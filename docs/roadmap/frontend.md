...
- [ ] **場景 B — 水平擴展 = 吞吐線性成長**：
  ...
  - 依賴 `worker_attempts` 欄位（見 §4 共用備註）：畫面顯示每個 job 由哪台 worker 處理，一眼看出負載被多台分擔。
...
- [x] **後端 `worker_attempts` 欄位**（場景 A/B 讓畫面可讀）：`TranscriptionJob.worker_attempts`（`JSONField(default=list)`），`mark_running` 在 guard 後 append `{host, at}`——每次 (re)delivery 一筆，記錄完整接手序列（不只最後一次）。前端顯示認領歷史。
