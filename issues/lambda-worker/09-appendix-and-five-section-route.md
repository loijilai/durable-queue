...
- [x] 新增 `/appendix` 頁：eyebrow「APPENDIX」、標題「Beyond the Route」、無副標，兩章沿用原 HA 頁 chapter index 的樣式，各有可直接連結的錨點：
...
- [x] `/high-availability` route 與 HA 頁刪除，不做 redirect。Worker Crash 情境（demo job、polling、docker compose 操作說明）全部刪除；AuditTrail 元件保留（Queue 頁仍在用）。
- [x] Security 頁只剩 Infra 與 App 兩區，編號 01、02，標題「Security Control」不變。
- [x] nav 與頁尾的 THE ROUTE 欄移除 High Availability；首頁 route 區的 eyebrow 改為「FIVE SECTIONS, IN ORDER」，卡片重編為 01–05。
- [x] 頁尾新增一欄「APPENDIX」，兩個連結分別直達兩章的錨點。Appendix 不出現在 nav 與首頁。
- [x] 沒有任何頁面或元件再引用已刪除的 HA 頁；不再使用的樣式刪除。
- [x] `./scripts/verify.sh full` 通過（含前端 lint 與 build），並在本機瀏覽器確認 Appendix 兩章、錨點連結、Security 頁與五站路線正常顯示。
